    ...
    model.find(conditions, fields, options, function(error, result){
        if(error) {
            ...
            if(result.length!=0){
                ...
                callback({success: 1, flag: "find data success",datas:result});
            }   
            else{
...
exports.pageQuery = function (page, pageSize, Model, populate, queryParams, sortParams, callback) {
    ...
    async.parallel({
        ...
        $page.total = Math.floor((count - 1) / pageSize) + 1;//总页数
        $page.datas = results.records;
        callback(err, $page);
    });
};
